        ...
        await brandKnowledge.proposeRevision({
          ...
          provenance: {
            sources: [
              {
                source: "inferred",
                basis: "Generated by Sage from this account's own stored data (Company Truth research run).",
              },
            ],
            confidence: "medium",
            conflict: false,
          ...
          refId: updated.rows[0].report_id,
          client,
        });
      }
...
async function approve(req, res) {
  try {
    ...
    // Prompt 011 (B3): the brand_knowledge_revisions row is the AUTHORITATIVE
    // approval record; company_truth_reports.status is a derived mirror
    // flipped in the same transaction by the canonical helper. A pending
    // report WITHOUT a paired pending revision is a surfaced divergence
    // defect (409) — never silently repaired.
    const pendingReport = await db.query(
      `SELECT report_id FROM company_truth_reports
        WHERE brand_id = $1 AND status = 'pending_approval'`,
      [brand.brand_id],
    );
    if (!pendingReport.rows.length) {
      return res.status(409).json({ error: "There is no report awaiting approval." });
    }
    const pendingRevision = await db.query(
      `SELECT revision_id FROM brand_knowledge_revisions
        WHERE brand_id = $1 AND kind = 'company_truth_report' AND status = 'pending'`,
      [brand.brand_id],
    );
    if (!pendingRevision.rows.length) {
      return res.status(409).json({
        error:
          "Company Truth mirror divergence detected: a report is pending but its approval record is missing. This is surfaced as a defect and must be investigated — it is never silently repaired.",
      });
    }
    await brandKnowledge.approveRevision({
      brandId: brand.brand_id,
      userId: req.user.userId,
      revisionId: pendingRevision.rows[0].revision_id,
    });

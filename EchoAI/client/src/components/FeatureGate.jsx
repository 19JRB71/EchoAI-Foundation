...
// the children render. Otherwise a persuasive, tier-colored upgrade prompt is
// shown instead (purple for Professional, gold for Enterprise).
//
...
const FEATURE_COPY = {
  ...
  appointments: {
    title: "Appointments",
    blurb: "Let leads book time on your calendar with AI-assisted scheduling and reminders.",
  },
  followups: {
    title: "AI Follow-Up Sequences",
    blurb: "Automatically nurture every lead with multi-step email, SMS, and call sequences until they convert.",
  },
  feedback: {
...
export default function FeatureGate({
  ...
  const accent = accentColor(requiredTier);
    ...
    <div className="mx-auto max-w-xl">
      <div
        ...
        style={{
          ...
          backgroundImage: `linear-gradient(to bottom, ${accent}26, rgba(17,24,39,0.95))`,
        }}
      >
        <div
        ...
        <h2 className="text-xl font-bold text-gray-100">
          {copy.title} is a{" "}
          <span style={{ color: accent }}>{name}</span> feature
        </h2>
        {copy.blurb && <p className="mt-2 text-sm leading-relaxed text-gray-300">{copy.blurb}</p>}
        <p className="mt-4 text-sm text-gray-300">
          Upgrade to <span className="font-semibold" style={{ color: accent }}>{name}</span>
          {price != null && (
            <>
              {" "}
        ...
        <button
          onClick={onUpgrade}
          className="mt-6 rounded-lg px-6 py-2.5 text-sm font-semibold text-gray-900 shadow transition hover:brightness-110"

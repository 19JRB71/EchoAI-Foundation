...
| AI Sales Agent | `/api/sales-agent` | **Platform-level, admin-only** (EchoAI selling itself — separate from the per-brand `/api/phone` agent). "Echo" answers EchoAI's OWN dedicated inbound demo line, qualifies prospects, scores interest 1-10 each turn, and closes; three-way call support: at interest≥7 (once, `invite_sent`) or manual Join, the owner is SMS'd a `/join/:callId` link that bridges owner + prospect into a Twilio `<Conference>`. "Hey Echo" co-pilot mid-call (sms=text owner / voice=speak). Creds from platform env `SALES_TWILIO_*` (degrade gracefully → offline msg / 503); settings in singleton `sales_agent_config` (`config_key='singleton'`). Webhooks (`/inbound`,`/voice/:callId`,`/status`,`/join/:callId` GET,`/conference/:callId`) are no-auth + X-Twilio-Signature-validated, always return 200 TwiML; admin routes are `auth → admin`. AI fail→502 (`AiError`, SDK errors lack `.status`); end-of-call AI summary is best-effort JSON → `outcome`(`VALID_SALES_OUTCOMES`)+`interest_score`(clamped 1-10). **Deviation:** platform sales prospects have no brand/lead row, so the brand-scoped follow-up SEQUENCE system doesn't apply — booking/follow-up tracked via `booked_demo`/`follow_up_scheduled` flags on `sales_calls`. Admin section: 4 tabs (Live Calls polling/Call History+transcript modal/Configuration/Performance). `sales_calls`+`sales_agent_config` (047) |

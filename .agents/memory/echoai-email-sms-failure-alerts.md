...
- Scheduled one-time email blasts: the cron claims each due campaign with a single-tx `FOR UPDATE SKIP LOCKED` (row stays 'scheduled' while sending — a crash rolls back and the next tick retries; no stale-'sending' rescue needed). Total failure flips 'scheduled'→'failed' via the guarded RETURNING; alert fires only after COMMIT.
- If a 'failed' campaign is retryable via the manual send path, that path must widen its recipient selection to include `delivery_status='failed'` when the campaign status is 'failed' — otherwise every recipient was already marked failed by the cron and the retry 400s with "no pending recipients". Safe because a failed recipient never received the message.
